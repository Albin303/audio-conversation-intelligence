...
@lru_cache(maxsize=1)
def _load_signal_weights() -> dict[str, dict[str, int]]:
    with open(BASE_DIR / "signal_weights.json", "r") as f:
        return json.load(f)


def _positive_signals() -> dict[str, int]:
    return _load_signal_weights().get("positive_signals", {})


def _negative_signals() -> dict[str, int]:
    return _load_signal_weights().get("negative_signals", {})
